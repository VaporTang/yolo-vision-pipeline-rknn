        ...
        for i in range(len(boxes)):
            for j in range(i + 1, len(boxes)):
                ...
                if iou > iou_threshold:
                    ...
                    print(f"  [Overlap] Label1: '{boxes[i]['label']}' vs Label2: '{boxes[j]['label']}'")
                    print(f"  IoU: {iou:.4f}")
...
def split_train_val(image_dir: str, label_dir: str, val_ratio: float = 0.2, seed: int = 42) -> Tuple[int, int]:
    """
...
def prepare_calibration_dataset(image_dir: str, output_file: str, num_images: int = 20) -> None:
    """
